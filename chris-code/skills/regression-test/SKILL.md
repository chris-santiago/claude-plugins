...
- **Fails on the old code, passes on the new.** Don't assume it would fail; prove it. Stash the fix, run the test, watch it fail, restore. Assumptions about "this would fail" are often wrong.
- **The assertion must discriminate:** true under correct behavior, false under the bug. One satisfied by *both* states proves nothing even when it passes. The trap is a check the broken state also happens to pass, e.g. "both value ranges are present" to catch two scales collapsed onto one, when the collapsed range still spans both endpoints. Name the broken state and confirm the assertion flips on it: assert a property the bug must violate, or simulate the broken output and watch the check fail. The weak "looks present" / "is not None" family is worse than no check; it manufactures confidence and hides the regression.
